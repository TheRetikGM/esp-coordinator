...
#ifndef TAG
#define TAG "no tag (commands_impl)"
#endif
...
;
template <>
struct zb_ncp::cmd_handle<GET_MODULE_VERSION>
    : immediate_cmd_process<GET_MODULE_VERSION>,
      general_status_res<GET_MODULE_VERSION, GET_MODULE_VERSION_resp_t> {
  static void process_status_res(ncp_generic_status_t &status,
                                 GET_MODULE_VERSION_resp_t *res) {
    res->fwVersion = 0x100;
...
struct zb_ncp::cmd_handle<NCP_RESET> : immediate_cmd_process<NCP_RESET>,
  ...
  static void process_status_arg(ncp_generic_status_t &status,
                                 uint8_t options) {
...
//     request: [],
//     response: [...commonResponse, {name: 'role', type: DataType.UINT8, typed:
//     DeviceType}],
// },
template <>
struct zb_ncp::cmd_handle<GET_ZIGBEE_ROLE>
    : immediate_cmd_process<GET_ZIGBEE_ROLE>,
      general_status_res<GET_ZIGBEE_ROLE, uint8_t> {
  static void process_status_res(ncp_generic_status_t &status,
                                 uint8_t *__attribute__((aligned(1))) res) {
...
// },
template <>
struct zb_ncp::cmd_handle<SET_ZIGBEE_ROLE>
...
;
template <>
struct zb_ncp::cmd_handle<SET_ZIGBEE_CHANNEL_MASK>
    : immediate_cmd_process<SET_ZIGBEE_CHANNEL_MASK>,
      general_status_arg<SET_ZIGBEE_CHANNEL_MASK,
                         SET_ZIGBEE_CHANNEL_MASK_arg_t> {
  static void process_status_arg(ncp_generic_status_t &status,
                                 const SET_ZIGBEE_CHANNEL_MASK_arg_t &arg) {
    if (arg.page != 0) {
      status = GENERIC_INVALID_PARAMETER_1;
    } else {
...
//     request: [],
//     response: [...commonResponse, {name: 'page', type: DataType.UINT8},
...
// },
template <>
struct zb_ncp::cmd_handle<SET_PAN_ID>
...
//     response: [...commonResponse, {name: 'mac', type: DataType.UINT8}, {name:
//     'ieee', type: DataType.IEEE_ADDR}],
// },
...
;
template <>
struct zb_ncp::cmd_handle<GET_LOCAL_IEEE_ADDR>
    : immediate_cmd_process<GET_LOCAL_IEEE_ADDR>,
      general_status_arg_res<GET_LOCAL_IEEE_ADDR, uint8_t,
                             GET_LOCAL_IEEE_ADDR_resp_t> {
  static void process_status_arg_res(ncp_generic_status_t &status, uint8_t arg,
                                     GET_LOCAL_IEEE_ADDR_resp_t *res) {
    if (arg != 0) {
...
// },
// template <>
// struct zb_ncp::cmd_handle<GET_TX_POWER> :
// immediate_cmd_process<GET_TX_POWER>,
// general_status_res<GET_TX_POWER,uint8_t> { 	static void
// process_status_res(ncp_generic_status_t& status,uint8_t* res) {
//     	*res = zb_get_tx_power();
//     }
// };
// // Set Transmit Power
...
// },
template <>
struct zb_ncp::cmd_handle<SET_TX_POWER>
...
// },
template <>
struct zb_ncp::cmd_handle<GET_RX_ON_WHEN_IDLE>
...
// },
template <>
struct zb_ncp::cmd_handle<SET_RX_ON_WHEN_IDLE>
...
// },
template <>
struct zb_ncp::cmd_handle<GET_JOINED>
...
// } __attribute__((packed));
// template <>
// struct zb_ncp::cmd_handle<GET_NWK_KEYS> :
// immediate_cmd_process<GET_NWK_KEYS>,
// 		general_status_res<GET_NWK_KEYS,GET_NWK_KEYS_resp_t> {
// 	static void process_status_res(ncp_generic_status_t& status,
// GET_NWK_KEYS_resp_t* res) {
...
//     request: [],
//     response: [...commonResponse, {name: 'extendedPanID', type:
//     BuffaloZBOSSDataType.EXTENDED_PAN_ID}],
// },
template <>
struct zb_ncp::cmd_handle<GET_EXTENDED_PAN_ID>
...
template <>
struct zb_ncp::cmd_handle<GET_COORDINATOR_VERSION>
...
extern "C" void
zb_aib_tcpol_set_update_trust_center_link_keys_required(uint8_t);
template <>
struct zb_ncp::cmd_handle<SET_TC_POLICY>
    : immediate_cmd_process<SET_TC_POLICY>,
      general_status_arg<SET_TC_POLICY, SET_TC_POLICY_arg_t> {
  static void process_status_arg(ncp_generic_status_t &status,
                                 const SET_TC_POLICY_arg_t &arg) {
    switch (arg.type) {
...
//     request: [{name: 'extendedPanID', type:
//     BuffaloZBOSSDataType.EXTENDED_PAN_ID}], response: [...commonResponse],
// },
template <>
struct zb_ncp::cmd_handle<SET_EXTENDED_PAN_ID>
    : immediate_cmd_process<SET_EXTENDED_PAN_ID>,
      general_status_arg<SET_EXTENDED_PAN_ID, zb_ext_pan_id_t> {
  static void process_status_arg(ncp_generic_status_t &status,
                                 const zb_ext_pan_id_t &arg) {
...
// },
template <>
struct zb_ncp::cmd_handle<SET_MAX_CHILDREN>
...
//             options: (payload, options) => (options.length =
//             payload.inputClusterCount),
//         },
//         {
//             name: 'outputClusters',
//             type: BuffaloZclDataType.LIST_UINT16,
//             options: (payload, options) => (options.length =
//             payload.outputClusterCount),
//         },
//     ],
//     response: [...commonResponse],
// },
...
template <>
struct zb_ncp::cmd_handle<AF_SET_SIMPLE_DESC>
    : immediate_cmd_process<AF_SET_SIMPLE_DESC> {
  static constexpr size_t resp_buffer_size = 2;

  static size_t process_immediate_d(const AF_SET_SIMPLE_DESC_arg_hdr_t& arg) {
    generic_response_t r;
    return process_immediate(&arg, sizeof(arg), (uint8_t*)&r, sizeof(r));
  }
...
struct NWK_FORMATION_arg_t {
  std::vector<SET_ZIGBEE_CHANNEL_MASK_arg_t> channels;
  uint8_t duration;
  uint8_t distrib_flag;
  uint16_t distrib_nwk;
  zb_ext_pan_id_t extended_pan_id;
};

struct NWK_FORMATION_resp_t : public generic_response_t {
  uint16_t nwk;
};
...
template<>
struct zb_ncp::cmd_handle<NWK_FORMATION>
    : delayed_cmd_process_direct<NWK_FORMATION, single_cmd_delayed,
                                 NWK_FORMATION_arg_t, NWK_FORMATION_resp_t> {
  static constexpr size_t resp_buffer_size = 4;
  static constexpr const char *name = "NWK_FORMATION";

  static int start_delayed(const NWK_FORMATION_arg_t& arg) {
    for (const auto& channel : arg.channels) {
      if (channel.page != 0) {
        return GENERIC_INVALID_PARAMETER;
      }

      ESP_LOGD(TAG, "zb_set_channel_mask: %08lx", (unsigned long)channel.mask);
      zb_ncp::set_channel_mask(channel.mask);
    }

    zb_set_extended_pan_id(arg.extended_pan_id);

    zb_ext_pan_id_t ext_pan_id;
    zb_get_extended_pan_id(ext_pan_id);
    ESP_LOGD(TAG,
             "set ext pan_id: %02x%02x%02x%02x%02x%02x%02x%02x / "
             "%02x%02x%02x%02x%02x%02x%02x%02x",
             int(arg.extended_pan_id[0]), int(arg.extended_pan_id[1]),
             int(arg.extended_pan_id[2]), int(arg.extended_pan_id[3]),
             int(arg.extended_pan_id[4]), int(arg.extended_pan_id[5]),
             int(arg.extended_pan_id[6]), int(arg.extended_pan_id[7]),
             int(ext_pan_id[0]), int(ext_pan_id[1]), int(ext_pan_id[2]),
             int(ext_pan_id[3]), int(ext_pan_id[4]), int(ext_pan_id[5]),
             int(ext_pan_id[6]), int(ext_pan_id[7]));
  ...
  static NWK_FORMATION_resp_t finish_delayed(int status) {
    return NWK_FORMATION_resp_t {
      generic_response_t {
        .category = STATUS_CATEGORY_NWK,
        .status = (ncp_generic_status_t)status,
      },
      static_cast<uint16_t>(status == 0 ? zb_get_pan_id() : 0)
    };
  }
};
...
template <>
struct zb_ncp::cmd_handle<NWK_START_WITHOUT_FORMATION>
  ...
  static uint16_t finish_delayed(int status, uint8_t *outdata,
                                 uint16_t outlen) {
...
//         ...commonResponse,
//         {name: 'len', type: DataType.UINT8},
//         {name: 'endpoints', type: BuffaloZclDataType.LIST_UINT8, options:
//         (payload, options) => (options.length = payload.len)}, {name: 'nwk',
//         type: DataType.UINT16},
//     ],
// },

template <>
struct zb_ncp::cmd_handle<ZDO_ACTIVE_EP_REQ>
    : request_cmd_process<ZDO_ACTIVE_EP_REQ, uint16_t, zb_zdo_active_ep_req_t,
                          zb_zdo_ep_resp_t> {
  using Base = request_cmd_process<ZDO_ACTIVE_EP_REQ, uint16_t,
                                   zb_zdo_active_ep_req_t, zb_zdo_ep_resp_t>;
  ...
  static uint16_t format_response(uint8_t *outdata,
                                  const zb_zdo_ep_resp_t *resp) {
    auto len = Base::format_response(outdata, resp);
    uint16_t ep_count = resp->ep_count;
    if (ep_count > 16) {
      ESP_LOGE(TAG, "S_ZDO_ACTIVE_EP_REQ: truncate endpoints %d",
               int(ep_count));
      ep_count = 16;
    }
    outdata[2] = ep_count;
    const uint8_t *ep_list = reinterpret_cast<const uint8_t *>(resp + 1);
    ESP_LOGI(TAG, "S_ZDO_ACTIVE_EP_REQ::format_response nwk_addr:%04x %d",
             resp->nwk_addr, ep_count);
    for (uint16_t i = 0; i < ep_count; ++i) {
      outdata[3 + i] = ep_list[i];
      ++len;
...
//   zb_uint8_t      status;   /*!< The status of the Desc_req command. @ref
//   zdp_status */ zb_uint16_t     nwk_addr; /*!< NWK address for the request */
//   zb_uint8_t      length;   /*!< Length of the simple descriptor */
// } ZB_PACKED_STRUCT
// zb_zdo_simple_desc_resp_hdr_t;

template <>
struct zb_ncp::cmd_handle<ZDO_SIMPLE_DESC_REQ>
    : request_cmd_process<ZDO_SIMPLE_DESC_REQ, zb_zdo_simple_desc_req_t,
                          zb_zdo_simple_desc_req_t, zb_zdo_simple_desc_resp_t> {
  using Base =
      request_cmd_process<ZDO_SIMPLE_DESC_REQ, zb_zdo_simple_desc_req_t,
                          zb_zdo_simple_desc_req_t, zb_zdo_simple_desc_resp_t>;
  ...
  static uint16_t format_response(uint8_t *outdata,
                                  const zb_zdo_simple_desc_resp_t *resp) {
    auto out = outdata;
    *out++ = STATUS_CATEGORY_ZDO;
    *out++ = resp->hdr.status;
    *out++ = resp->simple_desc.endpoint;
    *reinterpret_cast<uint16_t *>(out) = resp->simple_desc.app_profile_id;
    out += 2;
    *reinterpret_cast<uint16_t *>(out) = resp->simple_desc.app_device_id;
    out += 2;
    ...
    *out++ = output_count;
    ...
    for (uint16_t i = 0; i < input_count; ++i) {
      *reinterpret_cast<uint16_t *>(out) =
          resp->simple_desc.app_cluster_list[i];
      out += 2;
    }
    for (uint16_t i = 0; i < output_count; ++i) {
      *reinterpret_cast<uint16_t *>(out) =
          resp->simple_desc
              .app_cluster_list[i + resp->simple_desc.app_input_cluster_count];
      out += 2;
    }
    *reinterpret_cast<uint16_t *>(out) = resp->hdr.nwk_addr;
    out += 2;
...
//   */ zb_uint16_t          max_incoming_transfer_size; /*!< Maximum incoming
//   transfer size */ zb_uint16_t          server_mask;                /*!<
//   Server mask */ zb_uint16_t          max_outgoing_transfer_size; /*!<
//   Maximum outgoing transfer size */ zb_uint8_t desc_capability_field; /*!<
...
template <>
struct zb_ncp::cmd_handle<ZDO_NODE_DESC_REQ>
    : request_cmd_process<ZDO_NODE_DESC_REQ, zb_zdo_node_desc_req_t,
                          zb_zdo_node_desc_req_t, zb_zdo_node_desc_resp_t> {
  using Base =
      request_cmd_process<ZDO_NODE_DESC_REQ, zb_zdo_node_desc_req_t,
                          zb_zdo_node_desc_req_t, zb_zdo_node_desc_resp_t>;
  static constexpr size_t additional_buffer_size = 0;
  static constexpr const char *name = "ZDO_NODE_DESC_REQ";
  static void format_request(zb_zdo_node_desc_req_t &req,
                             const zb_zdo_node_desc_req_t &arg) {
    req = arg;
...
// },
template <>
struct zb_ncp::cmd_handle<ZDO_PERMIT_JOINING_REQ>
    : request_cmd_process<ZDO_PERMIT_JOINING_REQ,
                          zb_zdo_mgmt_permit_joining_req_param_t,
                          zb_zdo_mgmt_permit_joining_req_param_t,
                          zb_zdo_mgmt_permit_joining_resp_t> {
  using Base = request_cmd_process<ZDO_PERMIT_JOINING_REQ,
                                   zb_zdo_mgmt_permit_joining_req_param_t,
                                   zb_zdo_mgmt_permit_joining_req_param_t,
                                   zb_zdo_mgmt_permit_joining_resp_t>;
  static constexpr size_t additional_buffer_size = 0;
  ...
  static uint8_t start_request(uint8_t buf) {
    // ESP_LOGI(TAG,"S_ZDO_PERMIT_JOINING_REQ::start_request nwk_addr:%04x
    // time:%d",s_req.dest_addr,int(s_req.permit_duration));
...
template <>
struct zb_ncp::cmd_handle<ZDO_BIND_REQ>
    : request_cmd_process<ZDO_BIND_REQ, S_ZDO_BIND_REQ_arg_t,
                          zb_zdo_bind_req_param_t, zb_zdo_bind_resp_t> {
  using Base = request_cmd_process<ZDO_BIND_REQ, S_ZDO_BIND_REQ_arg_t,
                                   zb_zdo_bind_req_param_t, zb_zdo_bind_resp_t>;
  static constexpr size_t additional_buffer_size = 0;
  static constexpr const char *name = "ZDO_BIND_REQ";
  static constexpr bool request_is_data = false;
  static void format_request(zb_zdo_bind_req_param_t &req,
                             const S_ZDO_BIND_REQ_arg_t &arg) {
...
template <>
struct zb_ncp::cmd_handle<ZDO_UNBIND_REQ>
    : request_cmd_process<ZDO_UNBIND_REQ, S_ZDO_UNBIND_REQ_arg_t,
                          zb_zdo_bind_req_param_t, zb_zdo_bind_resp_t> {
  using Base = request_cmd_process<ZDO_UNBIND_REQ, S_ZDO_UNBIND_REQ_arg_t,
                                   zb_zdo_bind_req_param_t, zb_zdo_bind_resp_t>;
  static constexpr size_t additional_buffer_size = 0;
  static constexpr const char *name = "ZDO_UNBIND_REQ";
  static constexpr bool request_is_data = false;
  static void format_request(zb_zdo_bind_req_param_t &req,
                             const S_ZDO_UNBIND_REQ_arg_t &arg) {
    memcpy(req.src_address, arg.srcIeee, 8);
    req.src_endp = arg.srcEP;
    req.cluster_id = arg.clusterID;
    req.dst_addr_mode = arg.addrMode;
...
//         {name: 'ieee', type: DataType.IEEE_ADDR},
//         {name: 'nwk', type: DataType.UINT16},
//         {name: 'num', type: DataType.UINT8, condition: (payload, buffalo) =>
//         buffalo && buffalo.isMore()}, {name: 'startIndex', type:
//         DataType.UINT8, condition: (payload, buffalo) => buffalo &&
//         buffalo.isMore()}, {name: 'nwks', type:
//         BuffaloZclDataType.LIST_UINT16, options: (payload, options) =>
//         (options.length = payload.num)},
//     ],
// },

template <>
struct zb_ncp::cmd_handle<ZDO_IEEE_ADDR_REQ>
    : request_cmd_process<ZDO_IEEE_ADDR_REQ, zb_zdo_ieee_addr_req_param_t,
  ...
  static uint8_t start_request(uint8_t buf) {
    // ESP_LOGI(TAG,"S_ZDO_IEEE_ADDR_REQ::start_request nwk: %04x nwk_addr:
    // %04x",s_req.dst_addr,s_req.nwk_addr);
    return zb_zdo_ieee_addr_req(buf, &Base::req_cb);
  }
  static void handle_response(ResolveStrategy::request_t &req,
                              const zb_zdo_ieee_addr_resp_t *resp) {
    ...
    *out_cmd = req.cmd;
    ...
    outlen +=
        Cmd::format_response(reinterpret_cast<uint8_t *>(out_cmd + 1), resp);
    if (req.arg.request_type == 0x01) {
      auto ext =
          reinterpret_cast<const zb_zdo_ieee_addr_resp_ext_t *>(resp + 1);
      ...
      *dst++ = num;
      ...
      if (num) {
        auto ext2 =
            reinterpret_cast<const zb_zdo_ieee_addr_resp_ext2_t *>(ext + 1);
        *dst++ = ext2->start_index;
        ++outlen;
...
template <>
struct zb_ncp::cmd_handle<ZDO_NWK_ADDR_REQ>
    : request_cmd_process<ZDO_NWK_ADDR_REQ, zb_zdo_nwk_addr_req_param_t,
  ...
  static void handle_response(ResolveStrategy::request_t &req,
                              const zb_zdo_nwk_addr_resp_head_t *resp) {
    uint8_t outdata[Cmd::resp_buffer_size + sizeof(zb_ncp::cmd_t)];
    zb_ncp::cmd_t *out_cmd = reinterpret_cast<zb_ncp::cmd_t *>(outdata);
    *out_cmd = req.cmd;
    out_cmd->type = zb_ncp::RESPONSE;
    ...
    if (req.arg.request_type == 0x01) {
      ...
      *dst++ = num;
      ...
      if (num) {
        auto ext2 =
            reinterpret_cast<const zb_zdo_nwk_addr_resp_ext2_t *>(ext + 1);
        *dst++ = ext2->start_index;
        ++outlen;
        auto nwks = reinterpret_cast<const uint16_t *>(ext2 + 1);
        for (uint8_t i = 0; i < ext->num_assoc_dev; ++i) {
...
//     request: [{name: 'nwk', type: DataType.UINT16}],
//     response: [...commonResponse, {name: 'powerDescriptor', type:
//     DataType.UINT16}, {name: 'nwk', type: DataType.UINT16}],
// },
template <>
struct zb_ncp::cmd_handle<ZDO_POWER_DESC_REQ>
    : request_cmd_process<ZDO_POWER_DESC_REQ, zb_zdo_power_desc_req_t,
                          zb_zdo_power_desc_req_t, zb_zdo_power_desc_resp_t> {
  using Base =
      request_cmd_process<ZDO_POWER_DESC_REQ, zb_zdo_power_desc_req_t,
                          zb_zdo_power_desc_req_t, zb_zdo_power_desc_resp_t>;
  static constexpr size_t additional_buffer_size = 0;
  ...
  static uint16_t format_response(uint8_t *outdata,
                                  const zb_zdo_power_desc_resp_t *resp) {
    outdata[0] = STATUS_CATEGORY_ZDO;
    outdata[1] = resp->hdr.status;
    memcpy(&outdata[2], &resp->power_desc, sizeof(zb_af_node_power_desc_t));
    *reinterpret_cast<uint16_t *>(
        &outdata[2 + sizeof(zb_af_node_power_desc_t)]) = resp->hdr.nwk_addr;
...
//                                        OutClusterList.  */
//   zb_uint16_t      cluster_list[1]; /*!< Variable size: [num_in_clusters] +
//   [num_out_clusters]
...
//                                          OutClusterList is the desired list
//                                          to be matched by the Remote Device
//                                          (the elements of the
...
//   zb_uint8_t match_len; /*!< The count of endpoints on the Remote Device that
//   match the
//                           request criteria.*/
// }
// ZB_PACKED_STRUCT
...
template <>
struct zb_ncp::cmd_handle<ZDO_MATCH_DESC_REQ>
    : request_cmd_process<ZDO_MATCH_DESC_REQ, ZDO_MATCH_DESC_REQ_arg_t,
                          zb_zdo_match_desc_param_t, zb_zdo_match_desc_resp_t> {
  using Base =
      request_cmd_process<ZDO_MATCH_DESC_REQ, ZDO_MATCH_DESC_REQ_arg_t,
                          zb_zdo_match_desc_param_t, zb_zdo_match_desc_resp_t>;
  ...
  static size_t get_request_alloc_size(const ZDO_MATCH_DESC_REQ_arg_t &arg) {
    return sizeof(zb_zdo_match_desc_param_t) - 2 +
           (arg.inputClusterCount + arg.outputClusterCount) * 2;
  }
  static void format_request(zb_zdo_match_desc_param_t &req,
                             const ZDO_MATCH_DESC_REQ_arg_t &arg) {
    ...
    memcpy(req.cluster_list, arg.clusters,
           (arg.inputClusterCount + arg.outputClusterCount) * 2);
  }
  static uint16_t format_response(uint8_t *outdata,
                                  const zb_zdo_match_desc_resp_t *resp) {
    auto out = outdata;
    *out++ = STATUS_CATEGORY_ZDO;
    *out++ = resp->status;
    ...
    *reinterpret_cast<uint16_t *>(out) = resp->nwk_addr;
    out += 2;
    return out - outdata;
  }
};
...
//             type: BuffaloZBOSSDataType.LIST_TYPED,
//             typed: [
//                 {name: 'extendedPanID', type:
//                 BuffaloZBOSSDataType.EXTENDED_PAN_ID}, {name: 'ieee', type:
//                 DataType.IEEE_ADDR}, {name: 'nwk', type: DataType.UINT16},
...
template <>
struct zb_ncp::cmd_handle<ZDO_MGMT_LQI_REQ>
    : request_cmd_process<ZDO_MGMT_LQI_REQ, S_ZDO_MGMT_LQI_REQ_arg_t,
                          zb_zdo_mgmt_lqi_param_t, zb_zdo_mgmt_lqi_resp_t> {
  using Base =
      request_cmd_process<ZDO_MGMT_LQI_REQ, S_ZDO_MGMT_LQI_REQ_arg_t,
                          zb_zdo_mgmt_lqi_param_t, zb_zdo_mgmt_lqi_resp_t>;
  static constexpr bool request_is_data = false;
  static constexpr size_t additional_buffer_size =
      64 * sizeof(zb_zdo_neighbor_table_record_t);
  static constexpr const char *name = "ZDO_MGMT_LQI_REQ";
  static void format_request(zb_zdo_mgmt_lqi_param_t &req,
                             const S_ZDO_MGMT_LQI_REQ_arg_t &arg) {
    req.dst_addr = arg.nwk;
    req.start_index = arg.startIndex;
  }
  static uint8_t start_request(uint8_t buf) {
    // ESP_LOGI(TAG,"S_ZDO_MGMT_LQI_REQ::start_request dst_addr: %04x
    // start_index: %d",s_req.dst_addr,int(s_req.start_index));
    return zb_zdo_mgmt_lqi_req(buf, &Base::req_cb);
  }
  static uint16_t format_response(uint8_t *outdata,
                                  const zb_zdo_mgmt_lqi_resp_t *resp) {
    auto len = Base::format_response(outdata, resp);
    auto outwrite = &outdata[len];
    auto src =
        reinterpret_cast<const zb_zdo_neighbor_table_record_t *>(resp + 1);
    auto cnt = resp->neighbor_table_list_count;
    if (cnt > 64) {
      ESP_LOGE(TAG, "Truncate ZDO_MGMT_LQI_REQ %d",
               int(resp->neighbor_table_entries));
      cnt = 64;
      reinterpret_cast<zb_zdo_mgmt_lqi_resp_t *>(outdata)
          ->neighbor_table_list_count = cnt;
    }
    ESP_LOGD(
        TAG,
        "ZDO_MGMT_LQI_REQ format_response entries:%d start_index: %d len:%d",
        int(resp->start_index), int(cnt), int(cnt));
...
template <>
struct zb_ncp::cmd_handle<ZDO_MGMT_LEAVE_REQ>
    : request_cmd_process<ZDO_MGMT_LEAVE_REQ, ZDO_MGMT_LEAVE_REQ_args_t,
                          zb_zdo_mgmt_leave_param_t, zb_zdo_mgmt_leave_res_t> {
  using Base =
      request_cmd_process<ZDO_MGMT_LEAVE_REQ, ZDO_MGMT_LEAVE_REQ_args_t,
                          zb_zdo_mgmt_leave_param_t, zb_zdo_mgmt_leave_res_t>;
  static constexpr size_t additional_buffer_size = 0;
  static constexpr const char *name = "ZDO_MGMT_LEAVE_REQ";
  static void format_request(zb_zdo_mgmt_leave_param_t &req,
                             const ZDO_MGMT_LEAVE_REQ_args_t &arg) {
  ...
  static uint8_t start_request(uint8_t buf) {
    // ESP_LOGI(TAG,"S_ZDO_MGMT_LEAVE_REQ start_request device_address: "
    // IEEE_ADDR_FMT " dst_addr:%04x
    // ",IEEE_ADDR_PRINT(s_req.device_address),s_req.dst_addr);
...
template <>
struct zb_ncp::cmd_handle<ZDO_MGMT_BIND_REQ>
    : request_cmd_process<ZDO_MGMT_BIND_REQ, ZDO_MGMT_BIND_REQ_args_t,
                          zb_zdo_mgmt_bind_param_t, zb_zdo_mgmt_bind_resp_t> {
  using Base =
      request_cmd_process<ZDO_MGMT_BIND_REQ, ZDO_MGMT_BIND_REQ_args_t,
                          zb_zdo_mgmt_bind_param_t, zb_zdo_mgmt_bind_resp_t>;
  static constexpr size_t additional_buffer_size =
      64 * sizeof(zb_zdo_binding_table_record_t);
  static constexpr const char *name = "ZDO_MGMT_BIND_REQ";
  static void format_request(zb_zdo_mgmt_bind_param_t &req,
                             const ZDO_MGMT_BIND_REQ_args_t &arg) {
  ...
  // const dest = destAddrMode === 0x01 ? this.readUInt16() : destAddrMode ===
  // 0x03 ? this.readIeeeAddr() : undefined; const destEndpoint = destAddrMode
  // === 0x03 ? this.readUInt8() : undefined;
  ...
  static uint16_t format_response(uint8_t *outdata,
                                  const zb_zdo_mgmt_bind_resp_t *resp) {
    auto len = Base::format_response(outdata, resp);
    auto outwrite = &outdata[len];
    auto src =
        reinterpret_cast<const zb_zdo_binding_table_record_t *>(resp + 1);
    auto cnt = resp->binding_table_list_count;
    if (cnt > 64) {
      ESP_LOGE(TAG, "Truncate ZDO_MGMT_BIND_REQ %d", int(cnt));
      cnt = 64;
      reinterpret_cast<zb_zdo_mgmt_bind_resp_t *>(outdata)
          ->binding_table_list_count = cnt;
    }
    for (uint8_t i = 0; i < cnt; ++i) {
...
template <>
struct zb_ncp::cmd_handle<ZDO_MGMT_NWK_UPDATE_REQ>
    : request_cmd_process<ZDO_MGMT_NWK_UPDATE_REQ, zb_zdo_mgmt_nwk_update_req_t,
  ...
  static uint16_t
  format_response(uint8_t *outdata,
                  const zb_zdo_mgmt_nwk_update_notify_hdr_t *resp) {
    auto len = Base::format_response(outdata, resp);
    auto outwrite = &outdata[len];
    ...
    if (cnt > 64) {
      ...
      cnt = 64;
      reinterpret_cast<zb_zdo_mgmt_nwk_update_notify_hdr_t *>(outdata)
          ->scanned_channels_list_count = cnt;
    }
    for (uint8_t i = 0; i < cnt; ++i) {
      *outwrite++ = *src++;
    }
...
//         {name: 'profileID', type: DataType.UINT16},
//         {name: 'clusterID', type: DataType.UINT16},
//         {name: 'dstEndpoint', type: DataType.UINT8, condition: (payload) =>
//         [2, 3].includes(payload.dstAddrMode)},
...
//         //{name: 'aliasAddr', type: DataType.UINT16, condition: (payload) =>
//         payload.useAlias !== 0}, {name: 'aliasAddr', type: DataType.UINT16},
//         {name: 'aliasSequence', type: DataType.UINT8},
//         {name: 'data', type: BuffaloZclDataType.LIST_UINT8, options:
//         (payload, options) => (options.length = payload.dataLength)},
//     ],
//     response: [
//         ...commonResponse,
//         {name: 'ieee', type: DataType.IEEE_ADDR},
//         {name: 'dstEndpoint', type: DataType.UINT8, condition: (payload) =>
//         [2, 3].includes(payload.dstAddrMode)}, {name: 'srcEndpoint', type:
//         DataType.UINT8}, {name: 'txTime', type: DataType.UINT32}, {name:
//         'dstAddrMode', type: DataType.UINT8},
//     ],
// },
// const payload = {
//             paramLength: 20,
...
template <>
struct zb_ncp::cmd_handle<APSDE_DATA_REQ>
    : request_cmd_resolver<APSDE_DATA_REQ, APSDE_DATA_REQ_max_arg_t>,
      cmd_base<cmd_handle<APSDE_DATA_REQ>> {
  ...
  using ResolveStrategy =
      request_cmd_resolver<APSDE_DATA_REQ, APSDE_DATA_REQ_max_arg_t>;
  using Arg = apsde_data_req_arg_t;

  //         {name: 'ieee', type: DataType.IEEE_ADDR},
  //         {name: 'dstEndpoint', type: DataType.UINT8,
  //                 condition: (payload) => [2, 3].includes(payload.dstAddrMode)},
  //         {name: 'srcEndpoint', type: DataType.UINT8},
  //         {name: 'txTime', type: DataType.UINT32},
  //         {name: 'dstAddrMode', type: DataType.UINT8},

  static void handle_response(ResolveStrategy::request_t &req,
                              const zb_apsde_data_resp_t *resp) {
  ...
  static void aps_user_payload_callback(uint8_t param) {
    if (param) {
      ...
      uint8_t len = 0;
      auto data_ptr =
          static_cast<const uint8_t *>(zb_aps_get_aps_payload(param, &len));

      ESP_LOGD(
          TAG,
          "aps_user_payload_callback %d from %d hdr: %d addr:" IEEE_ADDR_FMT
          " tsn: %d len: %d:%p",
          ind->dst_endpoint, ind->src_endpoint, IEEE_ADDR_PRINT(ind->addr),
          int(ind->dst_addr_mode), int(data_ptr[1]), int(len), data_ptr);

      auto req = ResolveStrategy::resolve(data_ptr[1]);
      if (req) {
        ESP_LOGD(TAG, "%s::aps_user_payload_callback %d", Cmd::name,
                 int(data_ptr[1]));
        if (ind->status == 0) {
        ...
        ESP_LOGW(TAG, "%s not found request for response %d", Cmd::name,
                 int(data_ptr[1]));
      }
  ...
  static zb_ret_t start_request(uint8_t buf, ArgVar &arg,
    ...
    req.tsn = arg.data[1];
    ESP_LOGI(TAG,"<<<<< zb_aps_send_user_payload %d -> %d, %d len: %d",int(arg.hdr.base.src_endpoint),
      int(get_dst_endpoint(arg.hdr.base)),int(req.tsn),int(arg.hdr.dataLength));
    ESP_LOGI(TAG, "paramLength = %i", (int)arg.hdr.paramLength);
    ESP_LOGI(TAG, "dataLength = %i", (int)arg.hdr.dataLength);
    ESP_LOGI(TAG, "hdr.addr_data(c) = 0x%04x", dst_addr.addr_short);
    ESP_LOGI(TAG, "hdr.profile_id = 0x%x", (int)arg.hdr.base.profile_id);
    ESP_LOGI(TAG, "hdr.cluster_id = 0x%x", (int)arg.hdr.base.cluster_id);
    ESP_LOGI(TAG, "hdr.src_endpoint = 0x%x", (int)arg.hdr.base.src_endpoint);
    ESP_LOGI(TAG, "hdr.dst_endpoint = 0x%x", get_dst_endpoint(arg.hdr.base));
    ESP_LOGI(TAG, "hdr.radius = 0x%x", (int)arg.hdr.base.radius);
    ESP_LOGI(TAG, "hdr.addr_mode = 0x%x", (int)arg.hdr.base.addr_mode);
    ESP_LOGI(TAG, "hdr.tx_options = 0x%x", (int)arg.hdr.base.tx_options);
    ESP_LOGI(TAG, "hdr.use_alias = 0x%x", (int)arg.hdr.base.alias_src_addr);
    ESP_LOGI(TAG, "hdr.alias_seq_num = 0x%x", (int)arg.hdr.base.alias_seq_num);
    ESP_LOGI(TAG, "data = ");
    ESP_LOG_BUFFER_HEX_LEVEL(TAG, &arg.data, arg.hdr.dataLength, ESP_LOG_INFO);
    ESP_LOGI(TAG,"====");

    auto ret = zb_aps_send_user_payload(
        buf, dst_addr, arg.hdr.base.profile_id, arg.hdr.base.cluster_id,
        get_dst_endpoint(arg.hdr.base), arg.hdr.base.src_endpoint,
        arg.hdr.base.addr_mode,
        ZB_TRUE, // s->base.tx_options & 1 ? ZB_FALSE : ZB_TRUE,
        arg.data, arg.hdr.dataLength);
  ...
  static void process(const zb_ncp::cmd_t &cmd, const void *buffer,
                      size_t len) {
    if (len < sizeof(Arg)) {
    ...
    auto ret = zb_buf_get_out_delayed_ext(
        &do_request, ResolveStrategy::get_req_idx(req), len);

    //
    // auto ret =
    // zb_buf_get_out_delayed_ext(&do_request,ResolveStrategy::get_req_idx(req),sizeof(Req));
    if (ret != 0) {
      req->state = ResolveStrategy::request_t::S_NONE;
