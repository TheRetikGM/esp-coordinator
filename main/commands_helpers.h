...
#ifndef TAG
#define TAG "no tag (commmand helpers)"
#endif
...
template <typename Cmd> struct cmd_base {
  static constexpr ncp_status_category_t status_category =
      STATUS_CATEGORY_GENERIC;

  static void report_status(uint8_t status, generic_response_t &resp) {
    ESP_LOGD(TAG, "%s report_status: %s (%d)", Cmd::name,
             utils::get_zdp_status_str(status), status);
...
struct zb_ncp::immediate_cmd_process {
  ...
  static void process(const zb_ncp::cmd_t &cmd, const void *buffer, size_t len) {
    ...
    auto outlen =
        sizeof(outdata) +
        Cmd::process_immediate(buffer, len, &outdata[sizeof(zb_ncp::cmd_t)],
                               sizeof(outdata) - sizeof(zb_ncp::cmd_t));
    *out_cmd = cmd;
...
struct zb_ncp::delayed_cmd_process : public ResolveStrategyT<CmdId> {
  ...
  static void process(const zb_ncp::cmd_t &cmd, const void *buffer,
    ...
    if (res != 0) {
      ESP_LOGE(TAG, "%s:process_delayed:process failed start_delayed",
               Cmd::name);
      response(res);
    }
  }
  static bool response(int status) {
    if (ResolveStrategy::need_resolve()) {
      ...
      auto outlen =
          sizeof(outdata) +
          Cmd::finish_delayed(status, &outdata[sizeof(zb_ncp::cmd_t)],
                              sizeof(outdata) - sizeof(zb_ncp::cmd_t));
      zb_ncp::send_cmd_data(outdata, outlen);
      return true;
    }
...
struct zb_ncp::delayed_cmd_process_direct : public ResolveStrategyT<CmdId> {
  using Cmd = cmd_handle<CmdId>;
  using ResolveStrategy = ResolveStrategyT<CmdId>;

  inline static std::function<void(const Resp&)> m_callback = [](const Resp&){};

  static void process(const Arg& arg, const std::function<void(const Resp&)> callback) {
    ResolveStrategy::start_resolve(zb_ncp::cmd_t{
        .version = 0,
        .type = zb_ncp::REQUEST,
        .command_id = CmdId,
        .tsn = 0
  ...
  static bool response(int status) {
    ...
    zb_ncp::cmd_t cmd;
    ResolveStrategy::resolve(cmd);
    cmd.type = zb_ncp::RESPONSE;
    Resp resp = Cmd::finish_delayed(status);
    m_callback(resp);
...
template <command_id_t CmdId> struct single_cmd_delayed {
  inline static zb_ncp::cmd_t m_cmd = {};
  static void start_resolve(const zb_ncp::cmd_t &cmd) { m_cmd = cmd; }
  static bool need_resolve() { return m_cmd.command_id == CmdId; }
...
template<typename T>
struct ArrValueHolder {
  T value;

  ArrValueHolder(const T& arr) {
    memcpy(value, arr, sizeof(arr));
  }
};
...
struct zb_ncp::general_status_res {
  ...
  static size_t process_result(uint8_t *outdata, size_t outdata_size) {
    ...
    zb_ncp::cmd_handle<CmdId>::process_status_res(full_res->status.status,
                                                  &full_res->res);
  ...
  static std::conditional_t<std::is_array_v<R>, ArrValueHolder<R>, R>
  process_status_res_d() {
    FullRes res;
    res.status.category = STATUS_CATEGORY_GENERIC;
    res.status.status = GENERIC_OK;
    zb_ncp::cmd_handle<CmdId>::process_status_res(res.status.status, &res.res);

    return std::conditional_t<std::is_array_v<R>, ArrValueHolder<R>, R>{ res.res };
  }
};
...
struct zb_ncp::general_status_arg {
  ...
  static void process_status_arg_d(const Arg& arg) {
    FullRes fr;
    fr.status.category = STATUS_CATEGORY_GENERIC;
    fr.status.status = GENERIC_OK;
    zb_ncp::cmd_handle<CmdId>::process_status_arg(fr.status.status, arg);
  }
};
...
struct zb_ncp::general_status_arg_res {
  ...
  static size_t process_immediate(const void *inbuffer, size_t inlen,
    ...
    auto arg = static_cast<const Arg *>(inbuffer);
    zb_ncp::cmd_handle<CmdId>::process_status_arg_res(full_res->status.status,
                                                      *arg, &full_res->res);
    return sizeof(FullRes);
  }
  ...
  template<typename R = Res>
  static std::conditional_t<std::is_array_v<R>, ArrValueHolder<R>, R>
  process_status_arg_res_d(const Arg& arg) {
    FullRes fr;
    fr.status.category = STATUS_CATEGORY_GENERIC;
    fr.status.status = GENERIC_OK;
    zb_ncp::cmd_handle<CmdId>::process_status_arg_res(fr.status.status,
                                                      arg, &fr.res);
    return std::conditional_t<std::is_array_v<R>, ArrValueHolder<R>, R>{ fr.res };
  }
};
...
struct request_cmd_resolver {
  ...
  static uint16_t get_req_idx(const request_t *req) { return req - storage(); }
  static request_t &get_by_index(uint16_t idx) { return storage()[idx]; }
...
template <typename Resp> struct resp_parser {
  static inline uint8_t get_status(const Resp *resp) { return resp->status; }
  static inline uint8_t get_tsn(const Resp *resp) { return resp->tsn; }
};
...
struct zb_ncp::request_cmd_process : public request_cmd_resolver<CmdId, Arg> {
  ...
  static constexpr size_t resp_buffer_size =
      /*sizeof(generic_response_t) +*/ sizeof(Resp) +
      Cmd::additional_buffer_size;
  ...
  static void req_cb(uint8_t buf) {
    ...
    if (req) {
      ESP_LOGD(TAG, "%s::req_cb %d", Cmd::name,
               int(resp_parser<Resp>::get_tsn(resp)));
  ...
  static void format_request(Req &req, const Arg &arg) { req = arg; }
  static void on_request_started() {}
  static size_t get_request_alloc_size(const Arg &arg) { return sizeof(Req); }
  static void do_request(uint8_t buf, uint16_t req_arg) {
    Req *request_data;
    auto &req = ResolveStrategy::get_by_index(req_arg);
    if (Cmd::request_is_data) {
      ESP_LOGD(TAG, "%s::do_request zb_buf_initial_alloc req_idx: %d",
               Cmd::name, req_arg);
      request_data = static_cast<Req *>(
          zb_buf_initial_alloc(buf, Cmd::get_request_alloc_size(req.arg)));
    } else {
      ESP_LOGD(TAG, "%s::do_request zb_buf_alloc_tail req_idx: %d", Cmd::name,
               req_arg);
      request_data = static_cast<Req *>(
          zb_buf_alloc_tail(buf, Cmd::get_request_alloc_size(req.arg)));
    }
  ...
  static esp_err_t process(const zb_ncp::cmd_t &cmd, const void *buffer,
                           size_t len) {
    ...
    auto ret = zb_buf_get_out_delayed_ext(
        &do_request, ResolveStrategy::get_req_idx(req),
        Cmd::get_request_alloc_size(req->arg));
    if (ret != 0) {
      req->state = ResolveStrategy::request_t::S_NONE;
      report_failed(cmd, GENERIC_NO_RESOURCES);
      return ESP_OK;
    } else {
      ESP_LOGD(TAG, "%s::do_start req_idx: %d", Cmd::name,
               ResolveStrategy::get_req_idx(req));
    }
    return ESP_OK;
  }
};
